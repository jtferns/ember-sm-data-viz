...
// Modified by @jtferns to work as a standalone ember.js component
...
function cloudSprite(contextAndRatio, d, data, di) {
  ...
  while (--di >= 0) {
    ...
    let w2 = d.width,
        ...
        h2 = d.y1 - d.y0;
    // Zero the buffer
    for (let i = 0; i < h2 * w32; i++) sprite[i] = 0;
    x = d.xoff;
    if (x == null) return;
    y = d.yoff;
    let seen = 0,
    ...
    for (let j = 0; j < h2; j++) {
      for (let ii = 0; ii < w2; ii++) {
        let k = w32 * j + (ii >> 5),
            m = pixels[((y + j) * (cw << 5) + (x + ii)) << 2] ? 1 << (31 - (ii % 32)) : 0;
        sprite[k] |= m;
        seen |= m;
      }
      if (seen) seenRow = j;
...
export default function cloud() {
  ...
  function place(board, tag, bounds) {
    ...
    while (dxdy = s(t += dt)) { // eslint-disable-line no-cond-assign
      dx = ~~dxdy[0];
      dy = ~~dxdy[1];
